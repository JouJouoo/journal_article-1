...
$reportDir = "reports\dp_lcrl_10k_s012_carbon008_20260506"
...
foreach ($seed in 0, 1, 2) {
    Write-Host "[Carbon008] train seed=$seed"
    & $python -m dp_lcrl_rl.scripts.train.train_paper_mat `
        --experiment_name "dp_lcrl_10k_s012_carbon008_20260506_seed$seed" `
    ...
    Write-Host "[Carbon008] done seed=$seed"
}
...
runs = [
    Path("runs/dp_lcrl_10k_s012_carbon008_20260506_seed0/paper_training_summary.json"),
    Path("runs/dp_lcrl_10k_s012_carbon008_20260506_seed1/paper_training_summary.json"),
    Path("runs/dp_lcrl_10k_s012_carbon008_20260506_seed2/paper_training_summary.json"),
]
out_dir = Path("reports/dp_lcrl_10k_s012_carbon008_20260506")
out_dir.mkdir(parents=True, exist_ok=True)
out_png = out_dir / "training_curves_carbon008.png"
out_csv = out_dir / "training_curves_carbon008_stats.csv"
...
fig.suptitle("DP-LCRL Training Curves, carbon_price=0.08, 10k Steps, Seeds 0/1/2", fontsize=15)
...
with out_csv.open("w", encoding="utf-8") as f:
    f.write("metric,first30,last30,delta\n")
    for key in ["reward", "p2p", "grid_buy", "grid_sell", "carbon"]:
        ...
        first = float(np.mean(mean_raw[:30]))
        last = float(np.mean(mean_raw[-30:]))
        f.write(f"{key},{first:.8f},{last:.8f},{last-first:.8f}\n")

print(f"plot={out_png.resolve()}")
print(f"csv={out_csv.resolve()}")
'@ | & $python -

Write-Host "[Carbon008] all done"
