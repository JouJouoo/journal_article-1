import torch
import numpy as np
from dp_lcrl_rl.utils.util import update_linear_schedule
from dp_lcrl_rl.utils.util import get_shape_from_obs_space, get_shape_from_act_space
from dp_lcrl_rl.algorithms.utils.util import check
from dp_lcrl_rl.algorithms.mat.algorithm.ma_transformer import MultiAgentTransformer
...
class TransformerPolicy:
    ...
    def __init__(self, args, obs_space, cent_obs_space, act_space, num_agents, device=torch.device("cpu")):
        ...
        self.mask_mode = self._normalize_mask_mode(getattr(args, "mask_mode", "full"))
        ...
        self._use_id_emb = not getattr(args, "no_id_emb", False)
        self.transformer = MultiAgentTransformer(self.share_obs_dim, self.obs_dim, self.act_dim, num_agents,
                                                 ...
                                                 log_std_min=getattr(args, 'log_std_min', -2.0),
                                                 log_std_max=getattr(args, 'log_std_max', 2.0),
                                                 action_low=action_low, action_high=action_high,
                                                 use_id_emb=self._use_id_emb)
    ...
    def _normalize_mask_mode(mask_mode):
        ...
        if mode not in {"full", "obs_only"}:
            raise ValueError(f"Unsupported mask_mode={mask_mode}. Expected one of ['full', 'obs_only'].")
        return mode

    def _transformer_agent_mask(self, agent_mask, batch_size):
        if agent_mask is None:
            mask = np.ones((batch_size, self.num_agents), dtype=np.float32)
        elif isinstance(agent_mask, torch.Tensor):
            mask = agent_mask.reshape(-1, self.num_agents)
        else:
            mask = np.asarray(agent_mask, dtype=np.float32).reshape(-1, self.num_agents)

        if self.mask_mode == "obs_only":
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, masks, available_actions=None,
                    agent_mask=None, deterministic=False):
        """
        ...
        mask = self._transformer_agent_mask(agent_mask, cent_obs.shape[0])
    ...
    def get_values(self, cent_obs, obs, rnn_states_critic, masks, agent_mask=None):
        ...
        values = self.transformer.get_values(cent_obs, obs, mask)
    ...
    def evaluate_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, actions, masks,
        ...
        if available_actions is not None:
            available_actions = available_actions.reshape(-1, self.num_agents, self.act_dim)
        mask = self._transformer_agent_mask(active_masks, cent_obs.shape[0])

        action_log_probs, values, entropy = self.transformer(cent_obs, obs, actions, available_actions, mask)
    ...
    def restore(self, model_dir):
        ...
        if isinstance(transformer_state_dict, dict) and "state_dict" in transformer_state_dict:
            transformer_state_dict = transformer_state_dict["state_dict"]
        ...
        if skipped:
            print(
                "[Restore] Skipped {} incompatible keys (shape or name mismatch): {}".format(
    ...
    def pop_attn_stats(self):
        return self.transformer.pop_attn_stats()
